...
//	----------如果不是在選取文字，就處理超連結--------------
		int l;	char* url;
		if ((url = HyperLinkHitTest(point, l)))	//如果滑鼠點選到超連結
		{
...
//	超連結選單
	...
	*/
	...
	if (telnet)
	{
		...
		if ((link = HyperLinkHitTest(pt, len)))	//如果偵測到是超連結
		{
...
char* CTermView::HyperLinkHitTest(CPoint client_point, int& len)	//用來測試畫面上某點是否為超連結
//x,y為終端機行列座標，而不是滑鼠座標
{
